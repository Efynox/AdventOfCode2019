...
namespace AdventOfCodeApp.Day03
{
    class Passage
    {
    ...
    class Main : AbstractMain
    {
            ...
            foreach (string command in commands)
            {
                ...
                for (int i = 0; i < range; i++)
                {
                    ...
                    if (!map.ContainsKey(x))
                        map[x] = new Dictionary<int, List<Passage>>();

                    if (!map[x].ContainsKey(y))
                        map[x][y] = new List<Passage>();
        ...
        public string FindNearest()
        {
            ...
            int min = 999999999;
            foreach(var xPair in map)
            {
                foreach (var yPair in xPair.Value)
                {
                    if (xPair.Key != 0 && yPair.Key != 0
                        && yPair.Value.Exists(p => p.ShipCode == "A")
                        && yPair.Value.Exists(p => p.ShipCode == "B")
                        && (Math.Abs(xPair.Key) + Math.Abs(yPair.Key)) < min)
                    {
                        int dist = Math.Abs(xPair.Key) + Math.Abs(yPair.Key);
                        min = xPair.Key + yPair.Key;
                        result = string.Format("{0},{1} => {2}", xPair.Key, yPair.Key, dist);
                    }
                }
            }
            return result;
            ...
            foreach (var xPair in map)
            {
                foreach (var yPair in xPair.Value)
                {
                    if (xPair.Key != 0 && yPair.Key != 0
                        ...
                        && yPair.Value.Exists(p => p.ShipCode == "B"))
                    {
                        int dist = Min(yPair.Value.FindAll(p => p.ShipCode == "A").ConvertAll(p => p.Distance))
                                 + Min(yPair.Value.FindAll(p => p.ShipCode == "B").ConvertAll(p => p.Distance));
                        if (dist < min)
                        {
                            min = dist;
                            result = string.Format("{0},{1} => {2}", xPair.Key, yPair.Key, dist);
        ...
        {
            int xMin = Min(this.map.Keys);
            int xMax = Max(this.map.Keys);
            int yMin = 0;
            int yMax = 0;
            foreach (var xPair in map)
            {
                yMin = Math.Min(Min(xPair.Value.Keys), yMin);
                yMax = Math.Max(Max(xPair.Value.Keys), yMax);
            }
            ...
            for (int y = yMax; y >= yMin; y--)
            {
                for (int x = xMin; x <= xMax; x++)
                {
                    if (x == 0 && y == 0)
                        line.Append("O");
                    else if (map.ContainsKey(x) && map[x].ContainsKey(y))
                        line.Append(map[x][y].Count);
                    else
                        line.Append(".");
                }
