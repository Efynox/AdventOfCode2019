...
namespace AdventOfCodeApp.Day3B
{
    class Position
    {
    ...
    class Main : AbstractMain
    {
        ...
        private Position addPosition(int x, int y)
        {
            string key = string.Format("{0},{1}", x, y);
            if (positions.ContainsKey(key))
                return positions[key];
            return positions[key] = new Position { X = x, Y = y, Traces = new List<Trace>() };
        }
        ...
        public void Trace(string[] commands, string mark)
        {
            ...
            foreach (string command in commands)
            {
                ...
                for (int i = 0; i < range; i++)
                {
                    ...
                    current.Traces.Add(new Trace { Distance = distance, Ship = mark });
                }
            }
        }
        ...
        public string FindNearest()
        {
            ...
            {
                if(pos.X != 0 && pos.Y != 0 && pos.Traces.Exists(p => p.Ship == "A") && pos.Traces.Exists(p => p.Ship == "B"))
                {
                    int dist = Math.Abs(pos.X) + Math.Abs(pos.Y);
                    if (dist < min)
                    {
                        min = dist;
                        result = string.Format("{0},{1} => {2}", pos.X, pos.Y, dist);
                    }
                }
            }
            return result;
        ...
        public string FindFirst()
        {
            ...
            foreach (var pos in positions.Values)
            {
                if (pos.X != 0 && pos.Y != 0 && pos.Traces.Exists(p => p.Ship == "A") && pos.Traces.Exists(p => p.Ship == "B"))
                { 
                    int dist = Min(pos.Traces.FindAll(p => p.Ship == "A").ConvertAll(p => p.Distance))
                             + Min(pos.Traces.FindAll(p => p.Ship == "B").ConvertAll(p => p.Distance));
                    if (dist < min)
                    {
                        min = dist;
                        result = string.Format("{0},{1} => {2}", pos.X, pos.Y, dist);
        ...
        {
            var values = new List<Position>(positions.Values);
            int xMin = Min(values.ConvertAll(p => p.X));
            int xMax = Max(values.ConvertAll(p => p.X));
            int yMin = Min(values.ConvertAll(p => p.Y));
            int yMax = Max(values.ConvertAll(p => p.Y));
            ...
            for (int y = yMax; y >= yMin; y--)
            {
                for (int x = xMin; x <= xMax; x++)
                {
                    ...
                    if (x == 0 && y == 0)
                        ...
                        line.Append(position.Traces.Count);
                    else
                        line.Append(".");
                }
